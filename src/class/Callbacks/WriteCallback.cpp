...
WriteCallback::WriteCallback(
    int fd, SocketManager* manager, const std::string& header, int file_fd, int epoll_fd)
: Callback(fd)
, _epoll_fd(epoll_fd)
, _manager(manager)
, _header(header)
    ...
    if (!_headerDone)
    {
        ...
        if (written < 0)
        {
            _manager->getCallbackQueue().push(
                new ErrorCallback(_fd, _manager, _manager->getEpollFd(), FATAL));
            return;
        }
        _headerSent += written;
...
int WriteCallback::get_file_fd() const { return (_file_fd); }

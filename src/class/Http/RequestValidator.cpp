...
/*   Updated: 2025/05/31 14:27:01 by fsalomon         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void RequestValidator::validateBody()
{
    ...
    if ((_req.method == HttpRequest::METHOD_GET || _req.method == HttpRequest::METHOD_DELETE) && !_req.body.empty())
...
std::string RequestValidator::resolvePath(std::string relativePath, std::string root)
{
    ...
    if (!fullPath.empty() && fullPath[fullPath.size() - 1] != '/' && !relativePath.empty() && relativePath[0] != '/')
        fullPath += "/";

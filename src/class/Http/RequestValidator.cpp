...
/*   Updated: 2025/06/09 18:04:27 by anastruc         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
RequestValidator::RequestValidator(const HttpRequest &req,
	const ServerConfig &serverConfig) : _req(req), _serverConfig(serverConfig)
{
}
...
void RequestValidator::matchRoute()
{
	...
	if (!routes.empty())
	{
		for (std::map<std::string,
			RouteConfig>::const_iterator it = routes.begin(); it != routes.end(); ++it)
		{
			const std::string &prefix = it->first;
			if ((_req.path.compare(0, prefix.size(), prefix) == 0)
				|| (prefix[prefix.size() - 1] == '/'
					&& _req.path == prefix.substr(0, prefix.size() - 1)))
			{
...
void RequestValidator::validateMethod()
{
	...
	case HttpRequest::METHOD_DELETE:
		...
		break ;
	default:
		throw HttpException(400, "Bad Request: Unknown HTTP method",
			getErrorPage(400));
	}
	if (allowed.empty())
	{
...
void RequestValidator::validateBody()
{
	...
	max_size = _serverConfig.client_max_body_size;
	if (_matchedRoute)
	{
	...
	if ((_req.method == HttpRequest::METHOD_GET
			|| _req.method == HttpRequest::METHOD_DELETE) && !_req.body.empty())
		throw HttpException(400,
			"Bad Request: Unexpected body in GET or DELETE request",
			getErrorPage(400));
	...
	if (it != _req.headers.end() && !it->second.empty())
	{
		...
		if (isNumeric(val))
		{
			declaredLength = static_cast<size_t>(strtol(val.c_str(), NULL, 10));
			if (_req.body.size() != declaredLength)
				throw HttpException(400,
					"Bad Request: Body size does not match Content-Length",
					getErrorPage(400));
		}
	}
}

std::string RequestValidator::resolvePath(std::string relativePath,
	std::string root)
{
...
RouteConfig RequestValidator::getMatchedRoute() const
{
	return (*_matchedRoute);
}

bool RequestValidator::hasMatchedRoute() const
{
	return (_matchedRoute != NULL);
}

ServerConfig RequestValidator::getServerConfig() const
{
...
void RequestValidator::validateHeaders()
{
	...
	if (te_count > 1)
		throw HttpException(400,
			"Bad Request: Multiple Transfer-Encoding headers",
			getErrorPage(400));
	...
	for (SVSMAP::const_iterator it_h = _req.headers.begin(); it_h != _req.headers.end(); ++it_h)
	{
		if (it_h->second.size() > 1)
		{
			...
			{
				throw HttpException(400, "Bad Request: Duplicate header",
					getErrorPage(400));
			}
		}
	}

...
void ErrorHandler::setLogLevel(ErrorLevel level)
{
    _minLogLevel = level;
}
...
bool ErrorHandler::shouldShutdown() const
{
    return _shouldShutdown;
}

void ErrorHandler::resetShutdown()
{
    _shouldShutdown = false;
}

std::map<ErrorCategory, int> ErrorHandler::getErrorStats() const
{
    return _errorStats;
}
...
{
    for (std::map<ErrorCategory, int>::iterator it = _errorStats.begin();
         it != _errorStats.end(); ++it)
    {
        it->second = 0;
    }
}
...
std::string ErrorHandler::categoryToString(ErrorCategory category) const
{
    switch (category)
    {
        case SOCKET_ERROR:   return "SOCKET";
        case EPOLL_ERROR:    return "EPOLL";
        case CALLBACK_ERROR: return "CALLBACK";
        case TIMER_ERROR:    return "TIMER";
        case RESOURCE_ERROR: return "RESOURCE";
